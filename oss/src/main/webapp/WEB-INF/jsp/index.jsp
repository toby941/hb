	...
	<div id="content_main">
	 ...
	 <article class=" post type-post status-publish format-standard hentry category-a-eye tag-rom tag-rom article"  data-posttime="<fmt:formatDate value="${news.publishTime}" type="both" pattern="yyyy-MM-dd" />">
			<header>
				...
				<a href="/view/${news.id}" rel="bookmark" title="<c:out value="${news.title}" />" target="_blank"><img src="<c:out value="${news.picUrl}" />" alt="<c:out value="${news.title}" />" data-pinit="registered"></a>
				<h2 class="post_title">
